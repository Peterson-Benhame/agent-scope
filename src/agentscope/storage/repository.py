...
class Repository:
    ...
    def _upsert_source(self, conn, name: str) -> int:
        conn.execute(
            "INSERT INTO sources(name, type) VALUES(?, ?) ON CONFLICT(name) DO NOTHING",
            (name, name),
        )
    ...
    def _upsert_project(self, conn, path: str | None) -> int | None:
        if not path:
            return None
        conn.execute(
            "INSERT INTO projects(name, path) VALUES(?, ?) ON CONFLICT(path) DO UPDATE SET name=excluded.name",
            (self._project_name(path), path),
        )
        return int(conn.execute("SELECT id FROM projects WHERE path=?", (path,)).fetchone()[0])

    def upsert_model(self, conn, name: str | None, provider: str | None = None) -> int | None:
        if not name:
            return None
        ...
        return int(
            conn.execute(
                "SELECT id FROM models WHERE provider=? AND name=?", (provider_value, name)
            ).fetchone()[0]
        )
    ...
    def insert_message(self, session_id: int, turn_id: int | None, message: NormalizedMessage) -> int:
        key = self._event_key(
            "message", session_id, message.source_file, message.source_line, message.role, message.timestamp
        )
        with self.database.connect() as conn:
            conn.execute(
                """
            ...
            row = conn.execute("SELECT id FROM messages WHERE event_key=?", (key,)).fetchone()
            return int(row[0])
        ...
        **values,
        ...
        event_key = (
            ...
            if snapshot_key
                ...
                "cost", session_id, model_id, values.get("period_start"), values.get("period_end"),
                estimated_raw_cost_usd, observed_cost_usd, values.get("compression_savings_usd"),
                values.get("cache_savings_usd"), values.get("pricing_source"), values.get("pricing_version")
            )
        )
        with self.database.connect() as conn:
            conn.execute(
                ...
                """,
                (
                    session_id, model_id, values.get("period_start"), values.get("period_end"),
                    estimated_raw_cost_usd, observed_cost_usd,
                    values.get("estimated_cost_after_optimization_usd"),
                    values.get("compression_savings_usd"), values.get("cache_savings_usd"),
                    values.get("total_savings_usd"), values.get("pricing_source"),
                    values.get("pricing_version"), event_key,
                ),
            )
            row = conn.execute("SELECT id FROM costs WHERE event_key=?", (event_key,)).fetchone()
            return int(row[0])
    ...
    def upsert_turn(self, session_id: int, turn) -> int:
        with self.database.connect() as conn:
            ...
            conn.execute(
                """
                INSERT INTO turns(session_id, external_turn_id, started_at, ended_at, model_id, metadata_json)
                VALUES(?, ?, ?, ?, ?, ?)
                ON CONFLICT(session_id, external_turn_id) DO UPDATE SET
                    ...
                    metadata_json=excluded.metadata_json
                """,
                (session_id, turn.external_turn_id, turn.started_at, turn.ended_at, model_id,
    ...
    def insert_tool_call(self, session_id: int, turn_id: int | None, call) -> int:
        key = self._event_key("tool", session_id, call.source_file, call.source_line, call.external_call_id, call.name)
        with self.database.connect() as conn:
            provider = call.provider or ""
            category = call.category or "other"
            conn.execute(
                "INSERT INTO tools(name, provider, category) VALUES(?, ?, ?) ON CONFLICT(name, provider, category) DO NOTHING",
                (call.name, provider, category),
            )
            tool_id = int(conn.execute(
            ...
            conn.execute(
                ...
                """,
                (session_id, turn_id, tool_id, call.external_call_id, call.timestamp, call.duration_ms,
                 call.status, call.input_size, call.output_size, call.source_file, call.source_line,
                 json.dumps(call.metadata, ensure_ascii=False), key),
            )
            return int(conn.execute("SELECT id FROM tool_calls WHERE event_key=?", (key,)).fetchone()[0])

    def insert_token_usage(self, session_id: int, turn_id: int | None, usage) -> int:
        key = self._event_key("tokens", session_id, usage.source_file, usage.source_line, usage.timestamp)
        with self.database.connect() as conn:
            ...
            conn.execute(
                ...
                """,
                (session_id, turn_id, usage.timestamp, model_id, usage.input_tokens, usage.cached_input_tokens,
                 usage.cache_write_input_tokens, usage.output_tokens, usage.reasoning_output_tokens,
                 usage.total_tokens, usage.context_window, usage.source_file, usage.source_line, key),
            )
            return int(conn.execute("SELECT id FROM token_usage WHERE event_key=?", (key,)).fetchone()[0])

    def upsert_agent_evidence(self, session_id: int, evidence) -> int:
        with self.database.connect() as conn:
            conn.execute(
                "INSERT INTO agents(name, type) VALUES(?, ?) ON CONFLICT(name, type) DO NOTHING",
                (evidence.name, evidence.agent_type),
            )
            agent_id = int(conn.execute(
                "SELECT id FROM agents WHERE name=? AND type=?", (evidence.name, evidence.agent_type)
            ).fetchone()[0])
            parent_id = None
            if evidence.parent_name:
                conn.execute(
                    "INSERT INTO agents(name, type) VALUES(?, 'root') ON CONFLICT(name, type) DO NOTHING",
                    (evidence.parent_name,),
                )
                parent_id = int(conn.execute(
                    "SELECT id FROM agents WHERE name=? AND type='root'", (evidence.parent_name,)
                ).fetchone()[0])
            conn.execute(
                """
                ...
                """,
                (session_id, agent_id, parent_id, evidence.timestamp, evidence.evidence_type),
            )
            return agent_id
    ...
    def upsert_skill_evidence(self, session_id: int, evidence) -> int:
        with self.database.connect() as conn:
            ...
            conn.execute(
                "INSERT INTO skills(name, source, version) VALUES(?, ?, ?) ON CONFLICT(name, source, version) DO NOTHING",
                (evidence.name, source, version),
            )
            skill_id = int(conn.execute(
                "SELECT id FROM skills WHERE name=? AND source=? AND version=?", (evidence.name, source, version)
            ).fetchone()[0])
            conn.execute(
                """
                ...
                """,
                (session_id, skill_id, evidence.usage_type.value, evidence.timestamp, evidence.evidence_type),
            )
            return skill_id

    def insert_optimization(self, optimization, session_id: int | None = None) -> int:
        key = self._event_key(
            "optimization", optimization.source_file, optimization.source_line,
            optimization.timestamp, optimization.model, optimization.original_tokens,
            optimization.optimized_tokens, optimization.tokens_saved,
        )
        with self.database.connect() as conn:
            conn.execute(
                "INSERT INTO optimizers(name, version) VALUES(?, '') ON CONFLICT(name, version) DO NOTHING",
                (optimization.optimizer,),
            )
            optimizer_id = int(conn.execute(
                "SELECT id FROM optimizers WHERE name=? AND version=''", (optimization.optimizer,)
            ).fetchone()[0])
            ...
            conn.execute(
                ...
                """,
                (optimizer_id, session_id, optimization.timestamp, model_id, optimization.original_tokens,
                 optimization.optimized_tokens, optimization.tokens_saved, optimization.compression_percent,
                 optimization.cache_read_tokens, optimization.compression_savings_usd,
                 optimization.cache_savings_usd, optimization.observed_input_cost_usd,
                 optimization.confidence.value, optimization.source_file, optimization.source_line,
                 json.dumps(optimization.metadata, ensure_ascii=False), key),
            )
            return int(conn.execute("SELECT id FROM optimizations WHERE event_key=?", (key,)).fetchone()[0])

    def get_import_state(self, source: str, path: str):
        with self.database.connect() as conn:
            row = conn.execute(
                "SELECT * FROM import_state WHERE source=? AND path=?", (source, path)
            ).fetchone()
    ...
    def save_import_state(
        ...
        *,
        ...
        with self.database.connect() as conn:
            conn.execute(
                """
                INSERT INTO import_state(source, path, size, modified_at, content_hash, last_offset, status)
                VALUES(?, ?, ?, ?, ?, ?, ?)
    ...
    def record_import_error(
        self, source: str, file: str, line: int | None, error_type: str, error_message: str
    ) -> None:
        with self.database.connect() as conn:
            conn.execute(
                "INSERT INTO import_errors(source, file, line, error_type, error_message) VALUES(?, ?, ?, ?, ?)",
    ...
    def session_id_by_external(self, external_session_id: str) -> int | None:
        with self.database.connect() as conn:
            row = conn.execute(
                "SELECT id FROM sessions WHERE external_session_id=? ORDER BY id LIMIT 1", (external_session_id,)
            ).fetchone()
    ...
    def session_candidates(self):
        ...
        with self.database.connect() as conn:
            rows = conn.execute(
                """
                SELECT s.external_session_id, s.started_at, s.ended_at, m.name AS model, p.path AS project_path
                FROM sessions s
